...
int main(int argc, char** argv) {
    std::string file_name =
        (argc > 1) ? std::string(argv[1]) : "year2015/day1/data/input.txt";
    const std::string input = utils::read_file_to_string(file_name);

    auto begin = chrono_clock::now();
    const auto part_one = year2015::day1::solve_part1(input);
    ...
    const auto part_two = year2015::day1::solve_part2(input);
