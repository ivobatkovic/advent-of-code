...
def solve_part_one(input_string):
  ...
  for _ in range(100):
    ...
    # phase with k
    d = [abs(sum( (0,1,0,-1)[(i+1)//(k+1)%4]*d[i] for i in range(N) )) \
        % 10 for k in range(N)]
...
def solve_part_two(input_string):
  ...
  like [0,0,...,1,1], all the way until the middle digit in the number. Hence, 
  we can compute the number backwards if the offset is more than half the digit 
  '''
  ...
  if(len(d)*10000 < 2*d_skip):
    # Repeat the number 10000 times and, since we are in right half, chop the 
    # number up to the digits with the offset
    v = (10000*d)[d_skip:]

    # Loop through 100 FFTs
    for _ in range(100):
      # Go from the end of the list to the first number and compute the new 
      # values using the old ones
