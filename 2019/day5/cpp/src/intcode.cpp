...
Intcode::Intcode(bool reset, bool verbose) : 
    m_program(), m_input(), m_iter(0), m_base(0), m_output(0), m_reset(reset), 
    m_verbose(verbose), m_idle(false) {}

Intcode::Intcode(std::string file_name, std::vector<int64_t> input,
    bool reset, bool verbose) : m_program(), m_input(input), m_iter(0), 
        m_base(0), m_output(0), m_reset(reset), m_verbose(verbose), 
        m_idle(false) 
{
  // Read the program
  std::vector<std::vector<int64_t>> program_input;
  utils::readCSVFile(file_name,program_input);

  // Load the program into the intcode computer
  for (auto &p : program_input) {
    for (auto &px : p) {
      m_program.push_back(px);
    }
  }
}
...
void Intcode::operate(char oper, int64_t p1, int64_t p2, int64_t out)
{
  ...
  if (oper == '1') {
    ...
    if (p1 >= int64_t(m_program.size())) { m_program.resize(2*p1);}
