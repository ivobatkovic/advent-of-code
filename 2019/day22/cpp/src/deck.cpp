...
void Deck::parse(std::vector<std::pair<char, int>> instructions, int64_t &a,
                 int64_t &b) {
  a = 1;
  b = 0;
...
std::vector<std::pair<char, int>> Deck::read_shuffle() {
  ...
  auto input = utils::read_file(m_file_name);
  ...
  for (auto &line : input) {
    ...
    if (line.find("deal with increment") != std::string::npos) {
      ...
      instructions.push_back(std::make_pair('D', 0));
...
int Deck::part_one(int card) { return (m_a * card + m_b) % m_num_cards; }
...
int64_t Deck::part_two(int card, int64_t n_shuffles) {
  ...
  int128 B =
      mod(m_b * modular_divide(an - 1, m_a - 1, m_num_cards), m_num_cards);
