...
void Wires::intersect_wires(vector<vector<int>> &wire1,
  ...
  for (std::size_t i = 1; i < wire1.size() - 1; i++) {
    for (std::size_t j = 1; j < wire2.size() - 1; j++) {
      ...
      if (intersect(wire1[i], wire1[i + 1], wire2[j], wire2[j + 1], point,
                    steps)) {
...
bool Wires::intersect(vector<int> &A, vector<int> &B, vector<int> &C,
                      vector<int> &D, vector<int> &point_, int &steps_) {
  ...
  if ((x1max >= x2min) && (x1min <= x2max) && (y1max >= y2min) &&
      (y1min <= y2max)) {
    point_ = ((x1max - x1min) != 0) ? vector<int>{x2min, y1min}
                                    : vector<int>{x1min, y2min};

    int wire_steps = A[2] + C[2];

    int steps_to_point =
        std::abs(A[0] - point_[0]) + std::abs(A[1] - point_[1]) +
