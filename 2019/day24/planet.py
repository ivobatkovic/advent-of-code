...
class Planet:
    ...
    def __init__(self, input_, recursive=False):
        ...
        self.grid = self.read_map(input_)
            ...
            for col in range(self.w):
                for dx, dy in directions:
                    ...
                    neighbors = self.get_immediate_neighbors(
                        row_, col_, dx, dy, mid_x, mid_y
                    )

                    # Go through the list of found neighbors and append them:
                    # [level_difference, y_pos, x_pos]
                    for neighbor in neighbors:
                        neighbor_dict[row, col].append(neighbor)

        return neighbor_dict

    def get_immediate_neighbors(self, row_, col_, dx, dy, mid_x, mid_y):
        ...
        # Handle center of maze when it is recursive
            ...
            neighbors = self.get_recursive_center_neighbors(dx, dy)
        # Otherwise, we are inside the boundaries
        elif col_ >= 0 and col_ < self.w and row_ >= 0 and row_ < self.h:
            neighbors = [[0, row_, col_]]
        return neighbors

    def get_recursive_center_neighbors(self, dx, dy):
        # Left inside
        if dx == 1:
            neighbors = [[1, y, 0] for y in range(self.h)]
        # Right inside
        if dx == -1:
            neighbors = [[1, y, self.w - 1] for y in range(self.h)]
        # Top inside
        if dy == 1:
            neighbors = [[1, 0, x] for x in range(self.w)]
        # Bottom inside
        if dy == -1:
            neighbors = [[1, self.h - 1, x] for x in range(self.w)]
        return neighbors
    ...
    def update(self, n_updates=1):
        ...
        for update in range(n_updates):
            ...
            for level in range(-depth - 1, depth + 2):
                ...
                temp_level[level] = self.update_level(level)
        ...
        for row in range(self.h):
            ...
            for col in range(self.w):
                ...
                # Update empty tile
                    ...
                    if n_neighbors == 1 or n_neighbors == 2:
                        new_row += "#"
                    else:
                        new_row += "."
