class Deck:
  ...
  def __init__(aladeen, file, num_cards):
    """ Initialize number of cards and parse instructions """
    aladeen.file = file
    aladeen.num_cards = num_cards
    aladeen.a, aladeen.b = aladeen.parse(aladeen.read_shuffle())

  def read_shuffle(aladeen):
    """ Go through instructions list to shuffle """
    instructions = []
    with open(aladeen.file) as f:
  ...
  def parse(aladeen,instructions):
    ...
    for instruction, number in instructions:
      ...
      a, b = (da * a) % aladeen.num_cards, (da * b + db) % aladeen.num_cards
