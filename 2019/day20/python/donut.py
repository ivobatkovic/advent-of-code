  ...
  def find_portals(self):
    ...
    for y in range(h):
      for x in range(w):
        ...
        if key.isupper():
          # Look up
          if y - 2 > 0 and self.grid[y-1][x].isupper() \
                       and self.grid[y-2][x] == '.':
            yp, xp = y-2, x
            key_pair = self.grid[y-1][x]+key
          # Look down
          elif y + 2 < h and self.grid[y+1][x].isupper() \
                         and self.grid[y+2][x] == '.':
            yp, xp = y+2, x
            key_pair = key+self.grid[y+1][x]
          # Look left
          elif x - 2 > 0 and self.grid[y][x-1].isupper() \
                         and self.grid[y][x-2] == '.':
            yp, xp = y, x-2
            key_pair = self.grid[y][x-1]+key
          # Look right
          elif x + 2 < w and self.grid[y][x+1].isupper() \
                       and self.grid[y][x+2] == '.':
            yp, xp = y, x+2
