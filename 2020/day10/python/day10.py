...
def main():
    ...
    time_part1 = round((time.time() - t0) * 1e6)
    print(f"Solution to part one: {part1} (time taken {time_part1}[us])")

    t0 = time.time()
    part2 = solve_part2(input_)
    time_part2 = round((time.time() - t0) * 1e6)
    print(f"Solution to part two: {part2} (time taken {time_part2}[us])")
